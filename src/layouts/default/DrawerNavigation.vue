<template>
  <v-navigation-drawer app :rail="!showNavigationMenu" :width="280" permanent>
    ...
    <v-divider v-if="store.shortcuts.length > 0" class="my-2" />

    <!-- shortcuts section -->
    <v-list v-if="store.shortcuts.length > 0" lines="one" density="compact" nav>
...
import { store, type Shortcut } from "@/plugins/store";
...
const getShortcutsByType = (type: MediaType) => {
  return store.shortcuts.filter(shortcut => shortcut.itemType === type);
};
